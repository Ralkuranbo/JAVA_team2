...
public class WorkoutJournalApp {
    ...
    public static final Map<String, List<Exercise>> exercisesByPart = new LinkedHashMap<>();
    private static User user;
    private static LocalDate date;
    private static Map<String, Long> targetFrequency = new HashMap<>();
    ...
    static {
        ...
        deadlift.target = new String[]{"하부 등", "둔근", "코어"};
    ...
    private static void addWorkout(Scanner scanner) {
        ...
        if (targets != null) {
            long volume = (long) weight * reps * sets;
        for (String t : targets) {
            targetFrequency.put(t, targetFrequency.getOrDefault(t, 0L) + volume);
        }
        }
    }
    ...
    private static void loadFromFile(Scanner scanner) throws IOException {
        ...
        entries.clear();
        targetFrequency.clear();
        String line;
        String[] data;
        line = reader.readLine();
        data = line.split(",");
        boolean gend = data[0].equals("남자");
        ...
        while ((line = reader.readLine()) != null) {
            data = line.split(",");
            WorkoutEntry entry = WorkoutEntry.fromCSV(data);
            entries.add(entry);
            String[] targets = entry.getExercise().target;
            if (targets != null) {
                long volume = (long) entry.getWeight() * entry.getReps() * entry.getSets();
                for (String t : targets) {
                    targetFrequency.put(t, targetFrequency.getOrDefault(t, 0L) + volume);
                }
            }
    ...
    private static void showStatistics() {
        ...
        Map<LocalDate, List<WorkoutEntry>> entriesByDate = new TreeMap<>();
        for (WorkoutEntry entry : entries) {
    ...
    private static void recommendUndertrained() {
        ...
        for (String target : ALLTARGETS) {
            long count = targetFrequency.getOrDefault(target, 0L);
            if (count < minCount) {
                minCount = count;
            }
        }
        ...
        for (String target : ALLTARGETS) {
            if (targetFrequency.getOrDefault(target, 0L) == minCount) {
        ...
        for (String part : leastTrained) {
            System.out.println("- " + part + " (총 중량: " + targetFrequency.getOrDefault(part, 0L) + "kg)");

            List<String> suggestions = new ArrayList<>();
            for (Map.Entry<String, List<Exercise>> entry : exercisesByPart.entrySet()) {
                for (Exercise ex : entry.getValue()) {
                    if (ex.target != null) {
                        for (String t : ex.target) {
                            if (t.trim().equals(part.trim()) && !suggestions.contains(ex.getName())) {
            ...
            if (!suggestions.isEmpty()) {
                System.out.println("  추천 운동: " + String.join(", ", suggestions));
            } else {
                System.out.println("  해당 부위를 위한 운동이 정의되어 있지 않습니다.");
            }
        }
    }
}